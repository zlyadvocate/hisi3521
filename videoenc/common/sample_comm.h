...
HI_S32 SUBSTREAM_VENC_Start(  VENC_GRP VencGrp,VENC_CHN VencChn, PAYLOAD_TYPE_E enType, VIDEO_NORM_E enNorm, PIC_SIZE_E enSize, SAMPLE_RC_E enRcMode);
...
//HI_S32 VENC_StartGetMainStream(HI_S32 s32Cnt);//store main bitrate to file
HI_S32 VENC_StartGetMainStream(SAMPLE_VENC_GETSTREAM_PARA_S *spara);//store main bitrate to file
HI_S32 VENC_GetSubStream(HI_S32 s32Cnt);//get sub stream bitraye for streaming
...
HI_S32 HI_AUDIO_CreatTrdAiAenc(AUDIO_DEV AiDev, AI_CHN AiChn);//AI Capture TRD zly
...
HI_S32 HI_AUDIO_StartAi(AUDIO_DEV AiDevId, HI_S32 s32AiChnCnt, AIO_ATTR_S *pstAioAttr);//start AI zly 2015-05-14
