...
HI_S32 SAMPLE_COMM_VO_StartChn(VO_DEV VoDev,VO_PUB_ATTR_S *pstPubAttr,SAMPLE_VO_MODE_E enMode)
{
    ...
    {
        stChnAttr.stRect.s32X       = ALIGN_BACK((u32Width/u32Square) * (i%u32Square), 1);
        stChnAttr.stRect.s32Y       = ALIGN_BACK((u32Height/u32Square) * (i/u32Square), 1);
        stChnAttr.stRect.u32Width   = ALIGN_BACK(u32Width/u32Square, 1);
        stChnAttr.stRect.u32Height  = ALIGN_BACK(u32Height/u32Square, 1);
...
//zly 2013 -12-02
//set all chain to full size out
HI_S32 SAMPLE_COMM_VO_StartAllChn(VO_DEV VoDev,VO_PUB_ATTR_S *pstPubAttr,SAMPLE_VO_MODE_E enMode)
{
    HI_S32 i;
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32WndNum = 0;
    ...
    for (i=0; i<u32WndNum; i++)
    {
        stChnAttr.stRect.s32X       = u32Width;
        stChnAttr.stRect.s32Y       = u32Height;// ALIGN_BACK((u32Height/u32Square) * (i/u32Square), 2);
        stChnAttr.stRect.u32Width   = u32Width;//ALIGN_BACK(u32Width/u32Square, 2);
        stChnAttr.stRect.u32Height  = u32Height;//ALIGN_BACK(u32Height/u32Square, 2);
	...
	printf("mydebug:HI_MPI_VO_EnableChn dev=%d,chn=%d\n",VoDev,i);
        if (s32Ret != HI_SUCCESS)
        {
            SAMPLE_PRT("failed with %#x!\n", s32Ret);
            return HI_FAILURE;
...
//
HI_S32 SAMPLE_VO_CHN_show(VO_DEV VoDev,VO_PUB_ATTR_S *pstPubAttr,SAMPLE_VO_MODE_E enMode,HI_S32 channelindex)
{
    //init variable
	...
	u32Square=1;
  	...
  	stChnAttr.stRect.s32X       = ALIGN_BACK((u32Width/u32Square) * (i%u32Square), 1);
        ...
        stChnAttr.u32Priority       = 0;
        stChnAttr.bDeflicker        = HI_FALSE;
        s32ret = HI_MPI_VO_SetChnAttr(VoDev, channelindex, &stChnAttr);
	 if (s32ret != HI_SUCCESS)
	 {
		...
		{   s32ret = HI_MPI_VO_ChnShow(VoDev,channelindex);
			if (s32ret != HI_SUCCESS)
			{
