...
#define buf_readindex 2
#define buf_writedindex 3
...
int JSDsem_init(int buffsize,int index) { /* procedure to create and initialize semaphores and return semaphore id,
 assuming two semaphores defined in the given array of semaphores     */
    ...
    if ((semid = semget(index, 4, 0600 | IFLAGS)) < 0)
	{
		if( errno == EEXIST)
		{
            semid=semget(index,4,0600|IPC_CREAT);
			if(semid <0)
				JSDpr_error("shmget");
			else
				return semid;
    ...
    else
    {
        ...
        if (semctl(semid, buf_readindex, SETVAL, 0) < 0) {
            JSDpr_error("error in initializing third semaphore");
            exit(1);
        }

        if (semctl(semid, buf_writedindex, SETVAL, 0) < 0) {
            JSDpr_error("error in initializing fourth semaphore");
            exit(1);
        }
    }
	return semid;
...
void init_audio_buffer(audiobuffer* abuf, char * filepath,int buffercnt,int index) {
    ...
    abuf->buffer = (audiobitstream*) abuf->shared_memory; /* logical buffer starts at shared segment */
...
void audiobuffer_read(audiobuffer* abuf, audiobitstream* value){
    ...
    int index = semctl(abuf->semid, buf_readindex, GETVAL, 0);
    *value = abuf->buffer[index];
//	abuf->out = (abuf->out + 1) % abuf->buffercnt;
//	printf("Reader's report: item %2d == %2d\n", vbuf->out , value->videoseq);
    semctl(abuf->semid, buf_readindex, SETVAL, (index+1)%abuf->buffercnt);
