...
void *AUDIO_AiProc(void *parg)
{
	  HI_S32 s32Ret;
	    ...
	    printf("AiDev is %d,AiChn is %d\n",pstAiCtl->AiDev, pstAiCtl->AiChn);
...
HI_VOID*  mp3FileInit(HI_U8 channel ) {
	...
	strftime(mp3_file_name, 128, "%F-%H-%M-%S", newtime);
	strcat(mp3_file_name, ".mp3");

	sprintf(name, "chn%d_", channel);
	strcat(mp3_name, savepath);//
	strcat(mp3_name,name);
	strcat(mp3_name, mp3_file_name);
#if 1
	printf("%s\n", mp3_name);
#endif
	//create main stream file
	pFile = fopen(mp3_name, "wb");
	if (!pFile) {
...
void *SAMPLE_COMM_AUDIO_AiProc(void *parg)
{
    ...
    while (pstAiCtl->bStart)
    {
        ...
        if (FD_ISSET(AiFd, &read_fds))
        {
           ...
           // FillPacket((short *)stFrame.pVirAddr[0]);//
...
void *SAMPLE_COMM_AUDIO_AdecProc(void *parg)
{
    ...
    printf("%s: SAMPLE_COMM_AUDIO_AdecProc!\n", __FUNCTION__);
...
#if 0
        printf("mydebug:HI_MPI_ADEC_SendStream sucsessful %d bytes.\n",stAudioStream.u32Len);
#endif
...
/******************************************************************************
* function : get stream from file, and send it  to Adec
* *2014-02-24 by Zhangly
******************************************************************************/
void *SAMPLE_FFMPEG_AUDIO_AdecProc(void * parg)
{
    HI_S32 s32Ret;
    AUDIO_STREAM_S stAudioStream;
    HI_U32 u32Len = 640;
    ...
    while (HI_TRUE == pstAdecCtl->bStart)
    {
        ...
        u32ReadLen = fread(stAudioStream.pStream, 1, u32Len, pfd);//640byte
        if (u32ReadLen <= 0)
        {
            fseek(pfd, 0, SEEK_SET);/*read file again*/
            continue;
...
******************************************************************************/
HI_S32 HI_AUDIO_CreatTrdAiAenc(AUDIO_DEV AiDev, AI_CHN AiChn)
{
    SAMPLE_AI_S *pstAi = NULL;
    ...
    pstAi->AiChn = AiChn;
    pstAi->AencChn = 0;
   // pthread_create(&pstAi->stAiPid, 0, AUDIO_AiProc, pstAi);
    pthread_create(&pstAi->stAiPid, 0, SAMPLE_COMM_AUDIO_AiProc, pstAi);
...
printf("mydebug:SAMPLE_COMM_AUDIO_AoBindAdec->HI_MPI_SYS_Bind:adec->ao:%d.%d->%d.%d\n",
    stSrcChn.s32DevId,stSrcChn.s32ChnId,stDestChn.s32DevId,stDestChn.s32ChnId);
...
printf("mydebug:SAMPLE_COMM_AUDIO_AoBindAi->HI_MPI_SYS_Bind:ai->ao:%d.%d->%d.%d\n",
    stSrcChn.s32DevId,stSrcChn.s32ChnId,stDestChn.s32DevId,stDestChn.s32ChnId);
    return HI_MPI_SYS_Bind(&stSrcChn, &stDestChn);
}
...
printf("mydebug:SAMPLE_COMM_AUDIO_AencBindAi->HI_MPI_SYS_Bind:ai->aenc:%d.%d->%d.%d\n",
    stSrcChn.s32DevId,stSrcChn.s32ChnId,stDestChn.s32DevId,stDestChn.s32ChnId);
    return HI_MPI_SYS_Bind(&stSrcChn, &stDestChn);
}
...
* function : Start Ai
******************************************************************************/
HI_S32 HI_AUDIO_StartAi(AUDIO_DEV AiDevId, HI_S32 s32AiChnCnt, AIO_ATTR_S *pstAioAttr)
{
    HI_S32 i, s32Ret;
    ...
    for (i=0; i<s32AiChnCnt; i++)
    {
        ...
        printf("mydebug:SAMPLE_COMM_AUDIO_StartAi->HI_MPI_AI_EnableChn dev=%d,chn=%d\n",AiDevId,i);
    }
    return HI_SUCCESS;
}
/******************************************************************************
        ...
        AIO_ATTR_S *pstAioAttr, HI_BOOL bAnrEn, AUDIO_RESAMPLE_ATTR_S *pstAiReSmpAttr)
{
    ...
    for (i=0; i<s32AiChnCnt; i++)
    {
        ...
        if (HI_TRUE == bAnrEn)
        {
            ...
            printf("mydebug:SAMPLE_COMM_AUDIO_StartAi->HI_MPI_AI_EnableAnr dev=%d,chn=%d\n",AiDevId,i);
        }
        ...
        if (NULL != pstAiReSmpAttr)
        {
            ...
            printf("mydebug:SAMPLE_COMM_AUDIO_StartAi->HI_MPI_AI_EnableReSmp dev=%d,chn=%d\n",AiDevId,i);
        }
    }
        ...
        AIO_ATTR_S *pstAioAttr, AUDIO_RESAMPLE_ATTR_S *pstAoReSmpAttr)
{
    ...
    printf("mydebug:SAMPLE_COMM_AUDIO_StartAo->HI_MPI_AI_EnableChn dev=%d,chn=%d\n",AoDevId,AoChn);
...
HI_S32 SAMPLE_COMM_AUDIO_StartAenc(HI_S32 s32AencChnCnt, PAYLOAD_TYPE_E enType)
{
    ...
    for (i=0; i<s32AencChnCnt; i++)
    {
        ...
        printf("mydebug:SAMPLE_COMM_AUDIO_StartAenc->HI_MPI_AENC_CreateChn %d\n",AeChn);
    }
...
printf("mydebug:SAMPLE_COMM_AUDIO_StartAdec->HI_MPI_ADEC_CreateChn %d\n",AdChn);
    if (s32Ret)
    {
