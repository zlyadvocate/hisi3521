...
class FLVWriter {
 public:
  ...
  void writeDecoderConfiguration(uint32_t curr_time);
  std::vector<uint8_t> SEI_data;//
  void setSEIData(uint8_t* ptr, uint32_t size);
...
inline void FLVWriter::setSEIData(uint8_t* ptr, uint32_t size) {
	SEI_data.clear();
    std::copy(ptr,ptr+size, std::back_inserter(SEI_data));//
}
