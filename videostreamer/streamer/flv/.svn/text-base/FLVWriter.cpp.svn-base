...
//void FLVWriter::writeSPSMetaData(char *spsbuf,int timestamp)
//{
//	  BitStream bs_md;
//	  createMetaData(bs_md);
//	  //update SPS PPS data
//	  writeDecoderConfigurationRecord(bs_md);
//	  FLVTag tag;
//	  tag.makeScriptTag();
//
//	  tag.setTimeStamp(timestamp,0);
//	  tag.setData(bs_md.getPtr(), bs_md.size());
//
//	  metadata_offset = bs_md.size();
//	  {
//	    writeFLVTag(tag, bs_md);
//	  }
//	  metadata_size = bs_md.size() - metadata_offset;
//
//}
...
void FLVWriter::writeDecoderConfiguration(uint32_t curr_time){
	 BitStream bs_md;
	  FLVTag tag;
...
//  //
  if((tag.frame_type==FLV_VIDEOFRAME_KEY_FRAME)&&(tag.avc_packet_type!=FLV_AVC_SEQUENCE_HEADER))
  {
	  //write SEI data
	  s.putU32(ToBE32(SEI_data.size()));
	  s.putBytes(reinterpret_cast<unsigned char*>(&SEI_data[0]), SEI_data.size());
  }
  if(tag.avc_packet_type!=FLV_AVC_SEQUENCE_HEADER)
  s.putU32(ToBE32(tag.size));//zly  16:34
}
