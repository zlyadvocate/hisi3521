...
//    return false;
//  }
//  printf("audio_specific_config size is %d\n",header_size);
//  std::copy(header, header + header_size, std::back_inserter(audio_specific_config));
//  for (unsigned i=0; i<audio_specific_config.size(); i++)
//	  printf("config is %d",audio_specific_config.at(i));
//	printf("\n");
//
//  free(header);
//  header = NULL;
...
//  return true;
  int aacconfig_size;
  uint8_t* aacconfig;
  char HIAAC16K[]={0x14,0x8};
  char HIAAC32K[]={0x12,0x88};



   if(settings.samplerate == AV_AUDIO_SAMPLERATE_16000) //16Khz AAC
   {
       aacconfig_size = sizeof(HIAAC16K);
       aacconfig = (uint8_t*)(HIAAC16K);

   }
   else    // as PIC_CIF
   {
  	  aacconfig_size = sizeof(HIAAC32K);
       aacconfig = (uint8_t*)(HIAAC32K);
   }
 std::copy(aacconfig, aacconfig+(aacconfig_size), std::back_inserter(audio_specific_config));
 return true;
}
...
//  reinterpret_cast<unsigned char*> (&p->data[0])
  tag.setData(tag.bs.getPtr(), tag.bs.size());
#endif
#if 1
 // tag.bs.clear();//zly 2014-07-25
  tag.setData(p->data, written);
#endif
