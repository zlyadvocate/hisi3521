...
class RTMPWriter {
 public:
  ...
  int  RTMPwritepack(RTMP *r, const char *buf, int size);
 private:
  void reconnect();
  void close();
 private:
...
inline void RTMPData::putBytes(uint8_t* ptr, size_t nbytes) {
  std::copy(ptr, ptr+nbytes, data);
	// reinterpret_cast<unsigned char*>(&vid_pkt->data[0])
	//memcpy(data[0],(void *)ptr,nbytes);
  //std::copy(ptr, ptr+nbytes, reinterpret_cast<unsigned char*>(data[0]));
