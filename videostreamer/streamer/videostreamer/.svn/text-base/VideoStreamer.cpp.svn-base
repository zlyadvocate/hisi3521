...
bool VideoStreamer::start() {
  ...
  if(usesVideo()) {
    if(!video_enc.initialize()) {
    	 printf("video_enc.initialize failed\n");
      return false;
    }
    ...
    flv_writer.setVideoDataRate(300.81);
  }

  if(usesAudio()) {

    if(!audio_enc.initialize()) {
    	 printf("audio_enc.initialize failed\n");
      return false;
    }
    ...
    if(audio_settings.codec_id == AV_AUDIO_CODEC_AAC) {
      ...
      aac_enc.shutdown();//zly 2014-07-24
    }
...
bool VideoStreamer::addAudio(AVPacket* pkt) {
  ...
  if(enc_thread.state==ENCT_STATE_STARTED)
  {
...
uint8_t encoder_samplerate_to_flv(uint32_t v) {
  switch(v) {
    ...
    case AV_AUDIO_SAMPLERATE_16000: { return FLV_SOUNDRATE_16KHZ; }
    case AV_AUDIO_SAMPLERATE_32000: { return FLV_SOUNDRATE_32KHZ; }
    default: {
